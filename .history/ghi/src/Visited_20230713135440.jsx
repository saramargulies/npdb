...
function Visited() {
  ...
  const deleteItem = async (visited_list_item_id) => {
    const url = `http://localhost:8000/api/visited-lists/${visited_list_item_id}`
      ...
      <tbody className="border-top border-dark-subtle">
        {data.map(park => {
          ...
          <tr className="object-fit" key={ park.id }>
            ...
            <td>
              <button className="btn shadow btn-primary" onClick={deleteItem(.id)}>Delete</button>
            </td>
            <td>
